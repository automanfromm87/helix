...
class FileService:
    ...
    # Hard cap for client-supplied expiry (prevents abuse via the public route).
    # Internal callers may bypass via `internal=True`.
    PUBLIC_SIGNED_URL_CAP_MINUTES = 60 * 24  # 1 day
    EMBEDDED_SIGNED_URL_MINUTES = 60 * 24 * 7  # 7 days for screenshots embedded in chat history
    ...
    async def create_signed_url(
        ...
        `internal=True` skips the cap. Use it only for server-side embeds
        (e.g. screenshots inlined into SSE events) where the lifetime should
        match the chat history, not a user-shared link.
        """
        logger.info(
            "Create signed URL request: file_id=%s, user_id=%s, expire_minutes=%s, internal=%s",
            file_id, user_id, expire_minutes, internal,
        )
        ...
        if not internal:
            expire_minutes = min(expire_minutes, self.PUBLIC_SIGNED_URL_CAP_MINUTES)

        file_info = await self.get_file_info(file_id, user_id)
        if not file_info:
            logger.warning(
                "File not found or access denied for signed URL: file_id=%s, user_id=%s",
                file_id, user_id,
            )
        ...
        logger.info(
            "Created signed URL for user=%s file=%s (expires in %s min)",
            user_id, file_id, expire_minutes,
        )
        return signed_url
