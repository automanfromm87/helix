"""Plan/Task state-machine service.

Pure domain logic over PlanRepository + SessionRepository. Was originally
hosted under `app.application.services.plan_service`, which forced the
plan_act flow to import application from inside domain — a reverse
dependency. Moved here so plan_act stays self-contained inside domain.

The application layer keeps a re-export shim so existing callers (e.g.
plan_routes) work unchanged.
"""
