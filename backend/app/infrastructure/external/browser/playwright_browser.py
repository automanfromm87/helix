...
from app.infrastructure.external.llm import complete_text
...
class PlaywrightBrowser:
    ...
    async def _extract_content(self) -> Dict[str, Any]:
        ...
        return await complete_text(
            ...
            system=(
                "You are a professional web page information extraction assistant. "
                "Please extract all information from the current page content and "
    ...
    async def _extract_interactive_elements(self) -> List[str]:
        ...
        interactive_elements = await self.page.evaluate("""() => {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                // Only add data-helix-id attribute to elements that meet the conditions
                element.setAttribute('data-helix-id', `helix-element-${validElementIndex}`);
                                                        
                // Build selector - using only data-helix-id
                const selector = `[data-helix-id="helix-element-${validElementIndex}"]`;
    ...
    async def _get_element_by_index(self, index: int) -> Optional[Any]:
        """Get element by index using data-helix-id selector
        ...
        # Use data-helix-id selector
        selector = f'[data-helix-id="helix-element-{index}"]'
