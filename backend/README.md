# AI Helix Backend Service

English | [中文](README_zh.md)

AI Helix is an intelligent conversation agent system based on FastAPI and LangChain chat models. The backend adopts Domain-Driven Design (DDD) architecture, supporting intelligent dialogue, file operations, Shell command execution, and browser automation.
...
SANDBOX_IMAGE=simpleyyt/helix-sandbox          # Sandbox environment Docker image
SANDBOX_NAME_PREFIX=sandbox              # Sandbox container name prefix
SANDBOX_TTL_MINUTES=30                   # Sandbox container time-to-live (minutes)
SANDBOX_NETWORK=helix-network            # Docker network name for communication between sandbox containers

# Database configuration
POSTGRES_DSN=postgresql+asyncpg://helix:helix@localhost:5432/helix    # Postgres connection DSN
...
```bash
# Build Docker image
docker build -t helix-ai-agent .

# Run container
docker run -p 8000:8000 --env-file .env -v /var/run/docker.sock:/var/run/docker.sock helix-ai-agent
```
