...
| `API_BASE` | *(空)* | 否 | API 基础地址，用于指定模型服务的端点；留空则使用上游默认地址 |
...
| 配置项 | 默认值 | 是否必需 | 说明 |
|--------|--------|----------|------|
| `POSTGRES_DSN` | `postgresql+asyncpg://helix:helix@postgres:5432/helix` | 否 | Postgres 连接 DSN（必须是 `postgresql+asyncpg://` scheme） |
...
| `SANDBOX_IMAGE` | `simpleyyt/helix-sandbox` | 否 | Docker 沙箱镜像名称 |
| `SANDBOX_NAME_PREFIX` | `sandbox` | 否 | 沙箱容器名称前缀 |
| `SANDBOX_TTL_MINUTES` | `30` | 否 | 沙箱生存时间（分钟） |
| `SANDBOX_NETWORK` | `helix-network` | 否 | Docker 网络名称 |
...
| `GITHUB_REPOSITORY_URL` | `https://github.com/simpleyyt/ai-helix` | 否 | 前端 GitHub 按钮跳转地址 |
#### 密码认证配置
