...
| Configuration | Default Value | Required | Description |
|---------------|---------------|----------|-------------|
| `POSTGRES_DSN` | `postgresql+asyncpg://helix:helix@postgres:5432/helix` | No | Postgres connection DSN (must use `postgresql+asyncpg://` scheme) |
...
| `SANDBOX_IMAGE` | `simpleyyt/helix-sandbox` | No | Docker sandbox image name |
| `SANDBOX_NAME_PREFIX` | `sandbox` | No | Sandbox container name prefix |
| `SANDBOX_TTL_MINUTES` | `30` | No | Sandbox time-to-live in minutes |
| `SANDBOX_NETWORK` | `helix-network` | No | Docker network name |
...
| `GITHUB_REPOSITORY_URL` | `https://github.com/simpleyyt/ai-helix` | No | GitHub button target URL |
